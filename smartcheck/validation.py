...
import pandas as pd
from data_contracts.model import CSV, CheckResult, DataQuality
...
def _normalize_brazilian_decimals(
    ...
    aceito por `decimal.Decimal` ("1234.56"), só nas colunas tipadas como
    `Decimal` no schema — os arquivos de origem usam vírgula como separador
    decimal e o Python `Decimal` não entende esse formato."""
...
def validate_schema(
    ...
    try:
        ...
        return [], errors
    return [record.model_dump(by_alias=True) for record in parsed], []


def run_data_quality_checks(
    records: list[dict[str, Any]], data_quality: DataQuality
) -> list[CheckResult]:
    df = pd.DataFrame.from_records(records)
    return data_quality.run(df)
